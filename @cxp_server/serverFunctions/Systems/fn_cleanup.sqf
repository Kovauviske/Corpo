...
for "_i" from 0 to 1 step 0 do {
    ...
    uiSleep (60 * 60);
    {
        ...
        if ((_vehicleClass in ["Car","Air","Ship","Armored","Submarine"]) && {!(_protect)}) then {
            if (CXP_SETTINGS(getNumber,"save_vehicle_fuel") isEqualTo 1) then {_fuel = (fuel _veh);};
    ...
    {
        if ((typeOf _x) in ["Land_BottlePlastic_V1_F","Land_TacticalBacon_F","Land_Can_V3_F","Land_CanisterFuel_F", "Land_Can_V3_F","Land_Money_F","Land_Sleeping_bag_blue_folded_F"]) then {
            deleteVehicle _x;
        };
