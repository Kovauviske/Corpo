...
private _containers = [_query,2,true] call CXPDB_fnc_asyncCall;
...
private _containerss = [];
{
    ...
    _trunk = [_x select 3] call CXPDB_fnc_mresToArray;
    if (_trunk isEqualType "") then {_trunk = call compile format ["%1", _trunk];};
    _gear = [_x select 4] call CXPDB_fnc_mresToArray;
...
private _houses = [_query,2,true] call CXPDB_fnc_asyncCall;
