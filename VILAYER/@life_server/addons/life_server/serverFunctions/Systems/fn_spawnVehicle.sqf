...
if ((_vInfo select 5) isEqualTo 0) exitWith {
    ...
    [1,"STR_Garage_SQLError_Destroyed",true,[_vInfo select 2]] remoteExecCall ["cxp_fnc_broadcast",_unit];
};

if ((_vInfo select 6) isEqualTo 1) exitWith {
    serv_sv_use deleteAt _servIndex;
    [1,"STR_Garage_SQLError_Active",true,[_vInfo select 2]] remoteExecCall ["cxp_fnc_broadcast",_unit];
};
...
if (count _nearVehicles > 0) exitWith {
    serv_sv_use deleteAt _servIndex;
    [1,"STR_Garage_SpawnPointError",true] remoteExecCall ["cxp_fnc_broadcast",_unit];
    [_price,_unit_return] remoteExecCall ["cxp_fnc_garageRefund",_unit];
};

_query = format ["UPDATE vehicles SET active='1', damage='""[]""' WHERE pid='%1' AND id='%2'",_pid,_vid];

private _trunk = [(_vInfo select 9)] call CXPDB_fnc_mresToArray;
private _gear = [(_vInfo select 10)] call CXPDB_fnc_mresToArray;
private _damage = [call compile (_vInfo select 12)] call CXPDB_fnc_mresToArray;
...
[_vehicle] remoteExecCall ["cxp_fnc_addVehicle2Chain",_unit];
[_pid,_side,_vehicle,1] call CXPSV_fnc_keyManagement;
_vehicle lock 2;
//Reskin the vehicle
[_vehicle,(_vInfo select 8)] remoteExecCall ["cxp_fnc_colorVehicle",_unit];
...
if (CXP_SETTINGS(getNumber,"save_vehicle_virtualItems") isEqualTo 1) then {
    ...
    if (_wasIllegal) then {
        ...
        [1,"STR_NOTF_BlackListedVehicle",true,[_location,_name]] remoteExecCall ["cxp_fnc_broadcast",west];
...
if (CXP_SETTINGS(getNumber,"save_vehicle_fuel") isEqualTo 1) then {
    _vehicle setFuel (_vInfo select 11);
    }else{
    _vehicle setFuel 1;
};

if (count _gear > 0 && (CXP_SETTINGS(getNumber,"save_vehicle_inventory") isEqualTo 1)) then {
...
if (count _damage > 0 && (CXP_SETTINGS(getNumber,"save_vehicle_damage") isEqualTo 1)) then {
...
if ((_vInfo select 1) isEqualTo "civ" && (_vInfo select 2) isEqualTo "B_Heli_Light_01_F" && !((_vInfo select 8) isEqualTo 13)) then {
    [_vehicle,"civ_littlebird",true] remoteExecCall ["cxp_fnc_vehicleAnimate",_unit];
};

if ((_vInfo select 1) isEqualTo "cop" && (_vInfo select 2) in ["C_Offroad_01_F","I_MRAP_03_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","O_Heli_Light_02_unarmed_F"]) then {
    [_vehicle,"cop_offroad",true] remoteExecCall ["cxp_fnc_vehicleAnimate",_unit];
};

if ((_vInfo select 1) isEqualTo "med" && (_vInfo select 2) isEqualTo "C_Offroad_01_F") then {
    [_vehicle,"med_offroad",true] remoteExecCall ["cxp_fnc_vehicleAnimate",_unit];
};

if ((_vInfo select 14) isEqualTo 1) then {
    [1,"Seu veiculo esta pronto e assegurado."] remoteExecCall ["cxp_fnc_broadcast",_unit];
}else{
    [1,"Seu veiculo esta pronto."] remoteExecCall ["cxp_fnc_broadcast",_unit];
};
