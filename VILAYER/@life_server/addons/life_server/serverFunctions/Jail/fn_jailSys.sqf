...
if(_bad) then {//Load time from database
 ...
 _result = [_query,2] call CXPDB_fnc_asyncCall;
 _result = (_result select 0);
 _time = _result;
} else {
 _query = format["UPDATE players SET jail_time='%1' WHERE pid='%2'", _time, getPlayerUID _unit];
 _result = [_query,1] call CXPDB_fnc_asyncCall;
};
_id = owner _unit;
_ret = [_unit] call cxp_fnc_wantedPerson;
[_ret,_bad,_time] remoteExec ["cxp_fnc_jailMe",_id];