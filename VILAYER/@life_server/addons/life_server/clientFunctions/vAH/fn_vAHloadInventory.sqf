	...
	} else {
{
	...
	switch (_type) do {
		case "imenu": {
			_itemDisplayName = ([_item] call cxp_fnc_fetchCfgDetails) select 1;
			...
			if (_price isEqualTo 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Auction Price R$%1",[_price]call cxp_fnc_numberText];};
			};
		case "ymenu": {
