...
if (cxp_CXPHC_isActive) then {
    [_vehicle,false,(_this select 1),_storetext] remoteExec ["CXPHC_fnc_vehicleStore",CXP_HC];
} else {
    [_vehicle,false,(_this select 1),_storetext] remoteExec ["CXPSV_fnc_vehicleStore",RSERV];
};

hint localize "STR_Garage_Store_Server";
cxp_garage_store = true;
