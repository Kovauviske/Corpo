...
if (cxp_action_inUse) exitWith {hint localize "STR_NOTF_Action"};

_serviceCost = CXP_SETTINGS(getNumber,"service_chopper");
...
if (!alive (_search select 0) || (_search select 0) distance air_sp > 15) exitWith {cxp_action_inUse = false; hint localize "STR_Service_Chopper_Missing"};

CASH = CASH - _serviceCost;
[true] call cxp_fnc_hudUpdate;

if (!local (_search select 0)) then {
    [(_search select 0),1] remoteExecCall ["cxp_fnc_setFuel",(_search select 0)];
} else {
