...
if (isNil "cxp_fed_scam") then {
    cxp_fed_scam = "camera" camCreate [0,0,0];
    cxp_fed_scam camSetFov 0.5;
    cxp_fed_scam camCommit 0;
    "rendertarget0" setPiPEffect [0];
    cxp_fed_scam cameraEffect ["INTERNAL", "BACK", "rendertarget0"];
...
private _index = [_mode,_mTwPositions] call CXPSV_fnc_index;

if (_index isEqualTo -1) then {
    //Turn off
    cxp_fed_scam cameraEffect ["terminate", "back"];
    camDestroy cxp_fed_scam;
    _laptop setObjectTexture [0,""];
    cxp_fed_scam = nil;
} else {
    _temp = _mTwPositions select _index;
    cxp_fed_scam camSetPos (_dome modelToWorld (_temp select 1));
    cxp_fed_scam camSetTarget (_dome modelToWorld (_temp select 2));
    cxp_fed_scam camCommit 0;
};
