...
call {
    ...
    if !([_value] call CXPSV_fnc_isnumber) exitWith {
        hint localize "STR_NOTF_notNumberFormat";
    };
    if (parseNumber _value <= 0) exitWith {
        hint localize "STR_NOTF_enterAmountGive";
    };
    if !([false,_item, parseNumber _value] call cxp_fnc_handleInv) exitWith {
        hint localize "STR_NOTF_couldNotGive";
    };

    [_unit, _value, _item, player] remoteExecCall ["cxp_fnc_receiveItem", _unit];
