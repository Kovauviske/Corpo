...
if (typeOf _veh in ["Land_TentDome_F","Land_TentA_F"]) then {
	_armaze = getNumber(missionConfigFile >> "CxpBasic_Settings" >> "cxp_barracas" >> (typeOf _veh) >> "armazenamento");
	_usado = (_veh getVariable "Trunk") select 1;
	if (isNil "_usado") then {_usado = 0;};
	_veh_data = [_armaze,_usado];
} else {
	_veh_data = [_veh] call cxp_fnc_vehicleWeight;
};
