...
if (_vehTank isEqualTo []) exitWith {
    _vehicle setVariable ["fuelTank",[(getNumber(missionConfigFile >> "CxpCfgVehicles" >> (typeOf _vehicle) >> "vFuelSpace")),0],true];
...
if !(_trucks isEqualTo -1) exitWith {titleText[localize "STR_FuelTank_AnotherInUse","PLAIN"]; cxp_action_inUse = false};
...
if (_fuelFeedState isEqualTo 0) exitWith {titleText [localize "STR_FuelTank_FeedFull","PLAIN"]; cxp_action_inUse = false;};
...
if (_shortest < 1000) exitWith {titleText [localize "STR_FuelTank_PipeLine","PLAIN"]; cxp_action_inUse = false;};

private _pricem = getNumber(missionConfigFile >> "CxpBasic_Settings" >> "fuelTank_winMultiplier");
private _price = floor((((floor(_shortest / 100) * 100) / 1337) * _pricem) * 100) / 100;
private _win = 0;

_vehicle setVariable ["fuelTankWork",true,true];
_vehicle remoteExec ["cxp_fnc_soundDevice",RCLIENT];
cxp_action_inUse = false;
