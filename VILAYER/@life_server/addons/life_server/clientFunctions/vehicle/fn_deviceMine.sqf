...
cxp_action_inUse = true; //Lock out the interaction menu for a bit..

_weight = [_vehicle] call cxp_fnc_vehicleWeight;
...
_vehicle remoteExec ["cxp_fnc_soundDevice",RCLIENT]; //Broadcast the 'mining' sound of the device for nearby units.

cxp_action_inUse = false; //Unlock it since it's going to do it's own thing...
...
for "_i" from 0 to 1 step 0 do {
    ...
    _itemIndex = [_resource,_inv] call CXPSV_fnc_index;
    _weight = [_vehicle] call cxp_fnc_vehicleWeight;
    _random = 10 + round((random(10)));
    _sum = [_resource,_random,(_weight select 1),(_weight select 0)] call cxp_fnc_calWeightDiff; // Get a sum base of the remaining weight..
    ...
    if (local _vehicle) then {
        _vehicle setFuel (fuel _vehicle)-0;
    } else {
        [_vehicle,(fuel _vehicle)-0] remoteExec ["cxp_fnc_setFuel",_vehicle]; // removi a redução de gasolina colocando 0 na subtração
    };
    ...
    _itemWeight = ([_resource] call cxp_fnc_itemWeight) * _sum;
