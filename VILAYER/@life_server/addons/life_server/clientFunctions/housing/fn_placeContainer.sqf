...
if !(isNull _house) then {
    _message = 1;
    if (([player] call cxp_fnc_playerInBuilding) && {([_container] call cxp_fnc_playerInBuilding)}) then {
        _message = 2;
        if ((_house in cxp_vehicles) && !(isNil {_house getVariable "house_owner"})) then {
            _message = 3;
            if !(_isFloating) then {
                _message = 4;
                ...
                if (count _containers < (_houseCfg select 1)) then {
                    ...
                    if (cxp_CXPHC_isActive) then {
                        [_uid,_container] remoteExec ["CXPHC_fnc_addContainer",CXP_HC];
                    } else {
                        [_uid,_container] remoteExec ["CXPSV_fnc_addContainer",RSERV];
                    };
...
[true,_type,1] call cxp_fnc_handleInv;
