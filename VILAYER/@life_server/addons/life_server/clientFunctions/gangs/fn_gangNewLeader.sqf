...
if (_action) then {
    ...
    [_unit,group player] remoteExec ["CXPSV_fnc_clientGangLeader",_unit]; //Boot that bitch!

    if (cxp_CXPHC_isActive) then {
        [3,group player] remoteExec ["CXPHC_fnc_updateGang",CXP_HC]; //Update the database.
    } else {
        [3,group player] remoteExec ["CXPSV_fnc_updateGang",RSERV]; //Update the database.
    };
