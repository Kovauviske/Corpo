...
if (count _invs > 0) then {
    ...
    if (_illegal > 6000) then {
        ...
        if (cxp_CXPHC_isActive) then {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"482"] remoteExecCall ["CXPHC_fnc_wantedAdd",CXP_HC];
        } else {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"482"] remoteExecCall ["cxp_fnc_wantedAdd",RSERV];
        };

    };

    if (cxp_CXPHC_isActive) then {
        [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"481"] remoteExecCall ["CXPHC_fnc_wantedAdd",CXP_HC];
    } else {
        [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"481"] remoteExecCall ["cxp_fnc_wantedAdd",RSERV];
    };

    [0,"STR_Cop_Contraband",true,[(_civ getVariable ["realname",name _civ]),[_illegal] call cxp_fnc_numberText]] remoteExecCall ["cxp_fnc_broadcast",west];
} else {
...
if (_robber) then {
    [0,"STR_Cop_Robber",true,[(_civ getVariable ["realname",name _civ])]] remoteExecCall ["cxp_fnc_broadcast",RCLIENT];
};
