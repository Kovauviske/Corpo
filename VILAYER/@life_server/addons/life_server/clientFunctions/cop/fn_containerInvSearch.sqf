...
if (_value > 0) then {
    [0,"STR_NOTF_ContainerContraband",true,[[_value] call cxp_fnc_numberText]] remoteExecCall ["cxp_fnc_broadcast",RCLIENT];
    BANK = BANK + _value;
    [true] call cxp_fnc_hudUpdate;
    [1] call CXPSKT_fnc_updatePartial;
    _container setVariable ["Trunk",[[],0],true];
    [_container] remoteExecCall ["CXPSV_fnc_updateHouseTrunk",2];
} else {
