#include "\cxp_server\script_macros.hpp"
/*
    File: fn_houseMenu.sqf
    Author: Bryan "Tonic" Boardwine

	  Modified : Casperento

    Description:
    Building interaction menu
*/
disableSerialization;
private _curTarget = param [0,objNull,[objNull]];
if !(playerSide isEqualTo civilian) exitWith {hint "Voce nao pode interagir uma casa..."};
if (isNull _curTarget) exitWith {}; //Bad target
private _houseCfg = [(typeOf _curTarget)] call cxp_fnc_houseConfig;
if (count _houseCfg isEqualTo 0 && playerSide isEqualTo civilian) exitWith {};

if !(dialog) then {createDialog "pInteraction_Menu";};

private _Btn1 = CONTROL(37400,37450);
private _Btn2 = CONTROL(37400,37451);
private _Btn3 = CONTROL(37400,37452);
private _Btn4 = CONTROL(37400,37453);
private _Btn5 = CONTROL(37400,37454);
private _Btn6 = CONTROL(37400,37455);
private _Btn7 = CONTROL(37400,37456);
private _Btn8 = CONTROL(37400,37457);
private _Btn9 = CONTROL(37400,37458);
private _Btn10 = CONTROL(37400,37459);
cxp_pInact_curTarget = _curTarget;

if !(isNull cxp_pInact_curTarget) then {{_x ctrlShow false;} forEach [_Btn1,_Btn2,_Btn3,_Btn4,_Btn5,_Btn6,_Btn7,_Btn8,_Btn9,_Btn10];};

if (_curTarget in cxp_hideoutBuildings) exitWith {
    closeDialog 0;
    hint localize "STR_House_Hideout";
};

if (_curTarget isKindOf "House_F" && playerSide isEqualTo west) exitWith {

    private _vaultHouse = [[["Altis", "Land_Research_house_V1_F"], ["Tanoa", "Land_Medevac_house_V1_F"]]] call CXPSV_fnc_terrainSort;
    private _altisArray = [16019.5,16952.9,0];
    private _tanoaArray = [11074.2,11501.5,0.00137329];
    private _pos = [[["Altis", _altisArray], ["Tanoa", _tanoaArray]]] call CXPSV_fnc_terrainSort;

    if ((nearestObject [_pos,"Land_Dome_Big_F"]) isEqualTo _curTarget || (nearestObject [_pos,_vaultHouse]) isEqualTo _curTarget) then {

        _Btn1 ctrlShow true;
        _Btn1 ctrlSetText localize "STR_pInAct_Repair";
        _Btn1 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_repairDoor; closeDialog 0;";

        _Btn2 ctrlShow true;
        _Btn2 ctrlSetText localize "STR_pInAct_CloseOpen";
        _Btn2 buttonSetAction "[cxp_pInact_curTarget] call cxp_fnc_doorAnimate; closeDialog 0;";

    } else {
        if (!isNil {_curTarget getVariable "house_owner"}) then {
            _Btn1 ctrlShow true;
            _Btn1 ctrlSetText localize "STR_House_Raid_Owner";
            _Btn1 buttonSetAction "[cxp_pInact_curTarget] call cxp_fnc_copHouseOwner;";

            _Btn2 ctrlShow true;
            _Btn2 ctrlSetText localize "STR_pInAct_BreakDown";
            _Btn2 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_copBreakDoor; closeDialog 0;";

            _Btn3 ctrlShow true;
            _Btn3 ctrlSetText localize "STR_pInAct_SearchHouse";
            _Btn3 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_raidHouse; closeDialog 0;";

            if (player distance _curTarget > 3.6) then {
                _Btn3 ctrlEnable false;
            };

            _Btn4 ctrlShow true;
            _Btn4 ctrlSetText localize "STR_pInAct_LockHouse";
            _Btn4 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_lockupHouse; closeDialog 0;";

            _Btn5 ctrlShow true;
            _Btn5 ctrlSetText localize "STR_pInAct_BaterNaPorta";
            _Btn5 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_baternaporta; closeDialog 0;";
        } else {
            closeDialog 0;
        };
    };
};

if (!(_curTarget in cxp_vehicles) || isNil {_curTarget getVariable "house_owner"}) then {

    private _isHouse = false;

    _Btn1 ctrlShow true;
    if (isClass (missionConfigFile >> "Housing" >> worldName >> (typeOf _curTarget))) then {
        _Btn1 ctrlSetText localize "STR_pInAct_BuyHouse";
        _Btn1 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_buyHouse;";
        _isHouse = true;
    } else {
        _Btn1 ctrlSetText localize "STR_pInAct_BuyGarage";
        _Btn1 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_buyHouseGarage;";
    };

    if (!isNil {_curTarget getVariable "house_owner"}) then {_Btn1 ctrlEnable false;};

    if (_isHouse) then {
        if (getNumber (missionConfigFile >> "Housing" >> worldName >> (typeOf _curTarget) >> "canGarage") isEqualTo 1) then {
            _Btn2 ctrlShow true;
            _Btn2 ctrlSetText localize "STR_pInAct_GarageExt";
            _Btn2 buttonSetAction 'hint format [localize "STR_pInAct_GarageExtNOTF",[CXP_SETTINGS(getNumber,"houseGarage_buyPrice")] call cxp_fnc_numberText];';
        };
    };

} else {
    if (isClass (missionConfigFile >> "Garages" >> worldName >> (typeOf _curTarget))) then {
        _Btn1 ctrlShow true;
        _Btn1 ctrlSetText localize "STR_pInAct_SellGarage";
        _Btn1 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_sellHouse; closeDialog 0;";

        if (((_curTarget getVariable "house_owner") select 0) != getPlayerUID player) then {_Btn1 ctrlEnable false;};

        _Btn2 ctrlShow true;
        _Btn2 ctrlSetText localize "STR_pInAct_AccessGarage";
        _Btn2 buttonSetAction "[cxp_pInact_curTarget,""Car""] spawn cxp_fnc_vehicleGarage; closeDialog 0;";

        _Btn3 ctrlShow true;
        _Btn3 ctrlSetText localize "STR_pInAct_StoreVeh";
        _Btn3 buttonSetAction "[cxp_pInact_curTarget,player] spawn cxp_fnc_storeVehicle; closeDialog 0;";

    } else {
        _Btn1 ctrlShow true;
        _Btn1 ctrlSetText localize "STR_pInAct_SellHouse";
        _Btn1 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_sellHouse; closeDialog 0;";

        if (((_curTarget getVariable "house_owner") select 0) != (getPlayerUID player)) then {_Btn1 ctrlEnable false;};

        _Btn2 ctrlShow true;
        if (_curTarget getVariable ["locked",false]) then {
            _Btn2 ctrlSetText localize "STR_pInAct_UnlockStorage";
        } else {
            _Btn2 ctrlSetText localize "STR_pInAct_LockStorage";
        };
        _Btn2 buttonSetAction "[cxp_pInact_curTarget] call cxp_fnc_lockHouse; closeDialog 0;";

        _Btn3 ctrlShow true;
        if (isNull (_curTarget getVariable ["lightSource",objNull])) then {
            _Btn3 ctrlSetText localize "STR_pInAct_LightsOn";
        } else {
            _Btn3 ctrlSetText localize "STR_pInAct_LightsOff";
        };
        _Btn3 buttonSetAction "[cxp_pInact_curTarget] call cxp_fnc_lightHouseAction; closeDialog 0;";

        if (getNumber (missionConfigFile >> "Housing" >> worldName >> (typeOf _curTarget) >> "canGarage") isEqualTo 1 && {!(_curTarget getVariable ["blacklistedGarage",false])}) then {
            if (_curTarget getVariable ["garageBought",false]) then {
                _Btn4 ctrlShow true;
                _Btn4 ctrlSetText localize "STR_pInAct_SellGarage";
                _Btn4 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_sellHouseGarage; closeDialog 0;";

                if (((_curTarget getVariable "house_owner") select 0) != (getPlayerUID player)) then {_Btn4 ctrlEnable false;};

                _Btn5 ctrlShow true;
                _Btn5 ctrlSetText localize "STR_pInAct_AccessGarage";
                _Btn5 buttonSetAction "[cxp_pInact_curTarget,""Car""] spawn cxp_fnc_vehicleGarage; closeDialog 0;";

                _Btn6 ctrlShow true;
                _Btn6 ctrlSetText localize "STR_pInAct_StoreVeh";
                _Btn6 buttonSetAction "[cxp_pInact_curTarget,player] spawn cxp_fnc_storeVehicle; closeDialog 0;";
            } else {
                _Btn6 ctrlShow true;
                _Btn5 ctrlSetText localize "STR_pInAct_BuyGarage";
                _Btn5 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_buyHouseGarage; closeDialog 0;";
            };
        };

		_Btn6 ctrlShow true;
		_Btn6 ctrlSetText localize "STR_pInAct_BaterNaPorta";
        _Btn6 buttonSetAction "[cxp_pInact_curTarget] spawn cxp_fnc_baternaporta; closeDialog 0;";
		if (isNil {(_curTarget getVariable "house_owner")} OR ((_curTarget getVariable "house_owner") select 0) isEqualTo (getPlayerUID player)) then {_Btn6 ctrlEnable false;_Btn6 ctrlSetTooltip "Voce nao precisa bater na propria porta!";};
    };
};
