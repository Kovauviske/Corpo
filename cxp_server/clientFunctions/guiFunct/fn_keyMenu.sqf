...
for "_i" from 0 to (count cxp_vehicles)-1 do {
    _veh = cxp_vehicles select _i;
    if (!isNull _veh && alive _veh) then {
        _color = ((M_CONFIG(getArray,"CxpCfgVehicles",(typeOf _veh),"textures") select (_veh getVariable "CXP_VEH_color")) select 0);
