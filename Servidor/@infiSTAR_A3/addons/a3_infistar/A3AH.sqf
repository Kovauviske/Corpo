...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 18-10-2018 05-06-57 (v259)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='jmateustimm@gmail.com';
INFISTARVERSION='18-10-2018 05-06-57 - v259';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    jmateustimm@gmail.com - 18-10-2018 05-06-57 - v259 - %2 - %3',time,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [jmateustimm@gmail.com - 18-10-2018 05-06-57 - v259 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    jmateustimm@gmail.com - 18-10-2018 05-06-57 - v259 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [18-10-2018 05-06-57 - v259]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','b','f','9','5','b','9','9','f','c','1','e','b','c','5','f','3','6','4','9','1','a','1','4','a','3','3','6','c','1','d','7','7'];
...
if(_option isEqualTo 15)exitWith
{
	...
	if(_value)then
	{
		...
		[_array select 1] remoteExec ['cxp_fnc_restrain',_unit];
	}
	else
	{
		_unit setVariable['restrained',_value,true];
...
_log call FNC_A3_CONNECTLOG;
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=6905';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
"; }; _A3AHstring = _A3AHstring + "
	...
	if!(_admin)then
	{
		...
		_badclassesban =
		[
			'cxp_server','devcon','pooploop','rscrazzler','rsclistboxvg','vgdialog','niggers_die','ballers_die','grimbae','nss_admin_console','sspcm','balca_debug_tool','tonis_admin_menu_main',
			'oks_die'
		];
		_badclasseskick =
		[
	...
	if(_isNormal)then {
		...
		[_name,_puid] spawn {
			...
			_addCaseHDMGold = -1;
			_addCaseMEHold = -1;
			if(isNil 'cxp_fnc_keyHandler')then{cxp_fnc_keyHandler = {false};}else{if(typeName cxp_fnc_keyHandler != 'CODE')then{cxp_fnc_keyHandler = {false};};};
			...
			_actionTitle = toLower _actionTitle;
			{
				...
				"; if(_RDK)then{ _A3AHstring = _A3AHstring + "
					...
					"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
						...
						_kdID = (findDisplay 46) displayAddEventHandler ['KeyDown','_this call cxp_fnc_keyHandler;'];
					"; }; _A3AHstring = _A3AHstring + "
					"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
						_shouldbe = _shouldbe + 1;
					...
					if(alive player)then
					{
						...
						"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
							...
							"; if(_RKH)then{ _A3AHstring = _A3AHstring + "
								...
								player addEventHandler ['Killed', {_this spawn cxp_fnc_onPlayerKilled}];
							"; }; _A3AHstring = _A3AHstring + "
							"; if(_RHD)then{ _A3AHstring = _A3AHstring + "
								player removeAllEventhandlers 'HandleDamage';
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
			...
			"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
				...
				_rnd1 = round(random 99999);bafTuirY_ncioe9f = _rnd1;
				_rnd3 = round(random 99999);cxp_coplevel = _rnd2;
				_rnd4 = round(random 99999);cxp_fnc_MPexec = _rnd3;
				_rnd5 = round(random 99999);kniroNEKI0_yu = _rnd4;
			"; }; _A3AHstring = _A3AHstring + "
			_caeM1 = 0;
			_caeM2 = 0;
			...
			while{1==1}do
			{
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					if!(_rnd1 isEqualTo bafTuirY_ncioe9f)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',bafTuirY_ncioe9f,_rnd1])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo cxp_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',cxp_coplevel,_rnd2])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo cxp_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: cxp_fnc_MPexec [%1] should be [%2]',cxp_fnc_MPexec,_rnd3])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo kniroNEKI0_yu)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: kniroNEKI0_yu [%1] should be [%2]',kniroNEKI0_yu,_rnd4])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
				"; }; _A3AHstring = _A3AHstring + "
				"+_ninetwo+" = true;
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '13810x2';
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 18-10-2018 05-06-57 (v259) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_res = [VERSION_CHECK_URL_FULL+'&id=f0d036fe050b&li=bf95b99fc1ebc5f36491a14a336c1d77&pr=A3'] call FN_ARMA_FETCHDATA;
	...
	"; if(_CGM)then{ _A3AHstring = _A3AHstring + "
		...
		};
		{
			if!(_x in "+_MCS+")then
			{
				...
				if(_do)then
				{
					if(_lowMarker in ['gefmarker','deinvadder','swagmarker','dmcmarking','cxp_mpPacket_send'])then
					{
