...
// butao de comprar com a gangue nao deve aparecer para cop/samu
if (playerSide != civilian) then {ctrlShow [384115,false];};
...
if ((uiNamespace getVariable ["Weapon_Magazine",0]) isEqualTo 1 || {(uiNamespace getVariable ["Weapon_Accessories",0]) isEqualTo 1}) then {
    ...
    if ((uiNamespace getVariable ["Weapon_Magazine",0]) isEqualTo 1) then {
        ...
        {
            _bool = [_x] call cxp_fnc_levelCheck;
            if (_bool) then {
                _x params ["_var"];
    ...
    ((findDisplay 38400) displayCtrl 384115) ctrlSetText localize "STR_ATM_BuyWithGangButton";
            ...
            _itemArray pushBack M_CONFIG(getArray,"WeaponShops",_shop,"accs");
            {
                _y = _x;
                {
                    if (!(_x in _listedItems) && _x != "") then {
                        _iS = [_x,_y] call CXPSV_fnc_index;
                        if !(_iS isEqualTo -1) then {
                        _z = _y select _iS;
                            if (!((_z select 3) isEqualTo -1)) then {

                                _bool = [_z] call cxp_fnc_levelCheck;
                                if (_bool) then {

                                    _itemInfo = [_x] call cxp_fnc_fetchCfgDetails;
