...
if (!([false,"fuelFull",1] call cxp_fnc_handleInv)) exitWith {};
...
if (!alive player) exitWith {};
if (cxp_interrupted) exitWith {cxp_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"];};


switch (true) do {
    case (_vehicle isKindOF "LandVehicle"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.5] remoteExecCall ["cxp_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Air"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.2] remoteExecCall ["cxp_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Ship"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.35] remoteExecCall ["cxp_fnc_setFuel",_vehicle];
        } else {
...
[true,"fuelEmpty",1] call cxp_fnc_handleInv;
