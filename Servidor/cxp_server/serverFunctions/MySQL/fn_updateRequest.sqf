/*
    ...
    Information gets passed here from the client side file: session\fn_updateRequest.sqf
*/
...
_gear = [_gear] call CXPDB_fnc_mresArray;
_stats = [_stats] call CXPDB_fnc_mresArray;
_cash = [_cash] call CXPDB_fnc_numberSafe;
_bank = [_bank] call CXPDB_fnc_numberSafe;
_position = if (_side isEqualTo civilian) then {[_position] call CXPDB_fnc_mresArray} else {[]};

//Does something license related but I can't remember I only know it's important?
for "_i" from 0 to count(_licenses)-1 do {
    _bool = [(_licenses select _i) select 1] call CXPDB_fnc_bool;
    _licenses set[_i,[(_licenses select _i) select 0,_bool]];
};

_licenses = [_licenses] call CXPDB_fnc_mresArray;

//PLAYTIME
_playtime = [_uid] call CXPSV_fnc_getPlayTime;
_playtime_update = [];
{
...
} forEach CXPSV_fnc_playtime_values_request;
...
_playtime_update = [_playtime_update] call CXPDB_fnc_mresArray;
...
switch (_side) do {
    ...
    case civilian: {_query = format ["UPDATE players SET name='%1', cash='%2', bankacc='%3', civ_licenses='%4', civ_gear='%5', arrested='%6', civ_stats='%7', civ_alive='%8', civ_position='%9', playtime='%10' WHERE pid='%11'",_name,_cash,_bank,_licenses,_gear,[_this select 8] call CXPDB_fnc_bool,_stats,[_alive] call CXPDB_fnc_bool,_position,_playtime_update,_uid];};
