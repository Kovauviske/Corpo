...
if (player distance _house > 13) exitWith {cxp_action_inUse = false; titleText[localize "STR_House_Raid_TooFar","PLAIN"]};
...
_value = 0;
{
    ...
    if (ITEM_ILLEGAL(_var) isEqualTo 1) then {
        if (!(ITEM_SELLPRICE(_var) isEqualTo -1)) then {
            ...
            _houseInvVal = _houseInvVal - (([_var] call cxp_fnc_itemWeight) * _val);
...
if (_value > 0) then {
    [0,"STR_House_Raid_Successful",true,[[_value] call cxp_fnc_numberText]] remoteExecCall ["cxp_fnc_broadcast",RCLIENT];
    ...
    if (cxp_CXPHC_isActive) then {
        [_house] remoteExecCall ["CXPHC_fnc_updateHouseTrunk",CXP_HC];
    } else {
        [_house] remoteExecCall ["CXPSV_fnc_updateHouseTrunk",RSERV];
    };
} else {
