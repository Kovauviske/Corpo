...
for "_i" from 0 to 1 step 0 do {
...

    {
        ...
        if (_x isEqualTo player) then {
            _marcador = createMarkerLocal [format["%1_cop_pl_marc",_x],getPos _x];
            _marcador setMarkerColorLocal "ColorBlue"; // Marcador Azul
            _marcador setMarkerTypeLocal "mil_arrow2";
            _marcador setMarkerTextLocal "Você";
            _marcArr pushBack [_marcador,_x];
        };
        //Time
        if (side _x isEqualTo west && !(_x isEqualTo player)) then {
            _marcador = createMarkerLocal [format["%1_cop_terc_marc",_x],getPos _x];
            _marcador setMarkerColorLocal "ColorBLUFOR"; // Marcador Azul (BLUFOR)
