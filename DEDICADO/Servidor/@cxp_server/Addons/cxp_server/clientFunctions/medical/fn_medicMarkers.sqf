...
for "_i" from 0 to 1 step 0 do {
...

    {
        ...
        if (_x isEqualTo player) then {
            _marcador = createMarkerLocal [format["%1_med_pl_marc",_x],getPos _x];
        ...
        if (side _x isEqualTo independent && !(_x isEqualTo player)) then {
            _marcador = createMarkerLocal [format["%1_med_terc_marc",_x],getPos _x];
            _marcador setMarkerColorLocal "ColorGreen"; // Marcador Verde
