	...
	if (_type isEqualTo 0) then 
		{
			...
			{
				_itemName = ([_item] call cxp_fnc_fetchCfgDetails) select 1;
				_pic = ([_item] call cxp_fnc_fetchCfgDetails) select 2;
			};
	}; 

	if (_status isEqualTo 0)then {
	_myListbox lbAdd format [localize "STR_AH_LoadItems",_sellerName,[_price] call cxp_fnc_numberText,_itemName,[_time] call cxp_fnc_numberText,[_amount] call cxp_fnc_numberText];
