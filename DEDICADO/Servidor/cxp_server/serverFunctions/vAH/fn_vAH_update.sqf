...
private _expire = CXP_SETTINGS(getNumber,"ah_time_to_expire");
private _delete = CXP_SETTINGS(getNumber,"ah_time_to_delete");
...
switch (_switch) do {
	case 0: {
		_handle = {_id = _x select 0;_time = _x select 8;_time = round(_time - 1); if (_time < 1 && _x select 7 == 0) then {_status = 1} else {_status = _x select 7}; _toDel = round(_time + _delete); if (_toDel < 1) then {[1,_id] spawn CXPSV_fnc_vAH_update} else {[2,_id,_time,_status] spawn CXPSV_fnc_vAH_update};} forEach all_ah_items;
	...
	case 4: {
		...
		_items = [_items] call CXPDB_fnc_mresArray;
