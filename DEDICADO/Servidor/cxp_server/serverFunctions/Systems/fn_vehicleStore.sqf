...
_resourceItems = CXP_SETTINGS(getArray,"save_vehicle_items");

if (isNull _vehicle || isNull _unit) exitWith {cxp_impound_inuse = false; (owner _unit) publicVariableClient "cxp_impound_inuse";cxp_garage_store = false;(owner _unit) publicVariableClient "cxp_garage_store";}; //Bad data passed.
...
// save damage.
if (CXP_SETTINGS(getNumber,"save_vehicle_damage") isEqualTo 1) then {
    _damage = getAllHitPointsDamage _vehicle;
    _damage = _damage select 2;
    } else {
    _damage = [];
};
_damage = [_damage] call CXPDB_fnc_mresArray;
...
if (_impound) exitWith {
    if (count _vInfo isEqualTo 0) then  {
        ...
        (owner _unit) publicVariableClient "cxp_impound_inuse";
        ...
        _thread = [_query,1] call CXPDB_fnc_asyncCall;
...
if (count _vInfo isEqualTo 0) exitWith {
    [1,"STR_Garage_Store_NotPersistent",true] remoteExecCall ["cxp_fnc_broadcast",(owner _unit)];
    cxp_garage_store = false;
    (owner _unit) publicVariableClient "cxp_garage_store";
};

if !(_uid isEqualTo getPlayerUID _unit) exitWith {
    [1,"STR_Garage_Store_NoOwnership",true] remoteExecCall ["cxp_fnc_broadcast",(owner _unit)];
    cxp_garage_store = false;
    (owner _unit) publicVariableClient "cxp_garage_store";
};
...
if (CXP_SETTINGS(getNumber,"save_vehicle_virtualItems") isEqualTo 1) then {
    if (CXP_SETTINGS(getNumber,"save_vehicle_illegal") isEqualTo 1) then {
        private ["_isIllegal", "_blacklist"];
        _blacklist = false;
        _profileQuery = format ["SELECT name FROM players WHERE pid='%1'", _uid];
        _profileName = [_profileQuery, 2] call CXPDB_fnc_asyncCall;
        ...
        if (_blacklist) then {
            [_uid, _profileName, "481"] remoteExecCall["cxp_fnc_wantedAdd", RSERV];
...
// prepare
_trunk = [_trunk] call CXPDB_fnc_mresArray;
_cargo = [_cargo] call CXPDB_fnc_mresArray;
...
cxp_garage_store = false;
(owner _unit) publicVariableClient "cxp_garage_store";
[1,_storetext] remoteExecCall ["cxp_fnc_broadcast",(owner _unit)];
