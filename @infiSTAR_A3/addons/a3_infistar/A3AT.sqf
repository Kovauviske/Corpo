...
VERSION_DATE_IS = '18-10-2018 05-06-57';
infiSTAR_MAIN_CODE = "
...
fnc_LBDblClick_RIGHT = {
	...
	{
		if(_click == 'Give All Licenses')then{{missionnamespace setVariable[(_x select 0),true];} foreach cxp_licenses;};
		...
		FNC_VIRTUAL_ITEMSTHREAD = {
			...
			(findDisplay MAIN_DISPLAY_ID displayCtrl 65466) ctrlSetEventHandler['ButtonClick','
				...
				if(_msg != '''')then
				{
					...
					[''[true,''+str _click+'',''+str _value+''] call cxp_fnc_handleInv;'',SELECTED_TARGET_PLAYER] call admin_d0_target;
...
fnc_Altislife_money = {
	...
	if(_state == 'cash')then
	{
		bafTuirY_ncioe9f = bafTuirY_ncioe9f + _amount;
		_log = format['Cash %1K - %2K has been added.',bafTuirY_ncioe9f/1000,_amount/1000];
		[true] call cxp_fnc_hudUpdate;
		_log call FN_SHOW_LOG;
	};
	if(_state == 'bank')then
	{
		kniroNEKI0_yu = kniroNEKI0_yu + _amount;
		_log = format['Bank %1K - %2K has been added.',kniroNEKI0_yu/1000,_amount/1000];
		[true] call cxp_fnc_hudUpdate;
		_log call FN_SHOW_LOG;
	};
};
...
fnc_ReviveTarget = {
	...
	if(infiSTAR_MOD == 'AltisLife')then
	{
		...
		[profileName] remoteExec ['cxp_fnc_revived',_target];
	}
	else
	{
...
infiSTAR_A3Heal = {
	if(!isNil 'bafTuirY_ncioe9f')then{cxp_thirst = 100;cxp_hunger = 100;};
	if(infiSTAR_MOD == 'Epoch')then{
	...
	if(_veh isKindOf 'AllVehicles')exitWith{
		if((player distance _veh < 12) || ((_veh == vehicle player) && (vehicle player != player)))then
		{
			...
			if(_locked isEqualTo 2)then
			{
				...
				{
					_veh lock 0;
					[_veh,0] remoteExec ['cxp_fnc_lockVehicle',_veh];
					[_veh,'unlockCarSound'] remoteExec ['cxp_fnc_say3D',0];
				}
				else
				{
					_veh lock false;
			...
			else
			{
				...
				{
					_veh lock 2;
					[_veh,2] remoteExec ['cxp_fnc_lockVehicle',_veh];
					[_veh,'lockCarSound'] remoteExec ['cxp_fnc_say3D',0];
				}
				else
				{
					_veh lock true;
...
infiSTAR_A3Invulnerability = {
	fnc_A3_stopGod =
	{
		...
		if((infiSTAR_MOD == 'AltisLife') && (!isNil 'cxp_fnc_HandleDamage'))then
		{
		...
		if(!isNil 'ocxp_fnc_tazed')then{cxp_fnc_tazed = ocxp_fnc_tazed;};
	};
	...
	if(infiSTAR_A3GodRun == 0)then
	{
		...
		if(!isNil 'cxp_fnc_tazed')then
		{
			if(isNil 'ocxp_fnc_tazed')then
			{
				ocxp_fnc_tazed = cxp_fnc_tazed;
			};
			cxp_fnc_tazed = {};
		};
